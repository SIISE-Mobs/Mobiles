...
WHERE cibles_paiement.id_region=${region.id} AND cibles_paiement.date_paiement IS NOT NULL AND cibles_paiement.cin LIKE '%$cin%' ORDER BY cibles_paiement.cin ASC""");
  ...
  Future<List<CibleModelPaiement>> getCiblesPaiementPaid() async {
    await db;
    var region =
        Region.fromJson(jsonDecode(await _storage.read(key: 'region') ?? ''));
...
INNER JOIN districts ON districts.id = cibles_paiement.id_district 
LEFT JOIN subventions ON subventions.id = cibles_paiement.id_subvention 
WHERE cibles_paiement.id_region=${region.id} AND cibles_paiement.date_paiement IS NOT NULL AND cibles_paiement.id_subvention IS NOT NULL ORDER BY cibles_paiement.cin ASC""");
    return QueryResult.map((e) => CibleModelPaiement.fromMap(e)).toList();
  }
...
WHERE cibles.id_region=${region.id} AND cibles.cin LIKE '%$cin%' ORDER BY cibles.cin ASC""");
  ...
  Future<Subvention?> getLastSubventionDetails() async {
    ...
    var QueryResult = await _db!.rawQuery(
        "SELECT * FROM $subventionsTable WHERE id_region=${region.id} ORDER BY id DESC");
  ...
  Future<List<int>> getInformationsRegion() async {
    ...
    var cibles = await _db!.rawQuery(
        """SELECT count(*) as nbrCibles FROM cibles_paiement WHERE id_region=${region.id}""");
    cible = int.parse(cibles[0]['nbrCibles'].toString());
    ...
    var subvCible = await _db!.rawQuery(
        """SELECT count(*) as nbrCiblesSubv FROM '$subventionsTable' WHERE id_region=${region.id} """);
  ...
  Future<List<Subvention>> getSubventions() async {
    ...
    final List<Map<String, Object?>> QueryResult = await _db!.rawQuery(
        "SELECT * FROM $subventionsTable WHERE id_region=${region.id} ORDER BY date_sub DESC");
  ...
  Future<int> getCiblesData() async {
    ...
    List<Map> paiements = await _db!.rawQuery(
        'SELECT * FROM $ciblesPaiementTable WHERE id_region=${region.id}');

    // print(paiements.length);
    var cibles = await oService.getCiblesList(region.id);
  ...
  Future<CibleModelPaiement?> getCiblePaiementDetails(String? qrCode) async {
    ...
    String cId = codeSplit[0].toString().trim();
    String cCId = codeSplit[1].toString().trim();
    String cNom = codeSplit[2].toString().trim();
    String cCIN = codeSplit[3].toString().trim();

    var QueryResult = await _db!.rawQuery(
        "SELECT * FROM $ciblesPaiementTable WHERE cin LIKE '%$cCIN%' AND id='$cId' AND nom LIKE '%$cNom%' AND id_commune='$cCId' AND id_region=${region.id} ORDER BY id DESC");
